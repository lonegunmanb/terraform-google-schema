...
func ExtractAzureRMProviderSchema() (*ProviderSchema, error) {
	...
	r := schema.Schemas["registry.terraform.io/hashicorp/google"]

	_, versions, err := tf.Version(context.Background(), true)
	if err != nil {
		return nil, fmt.Errorf("error running version: %w", err)
	}
	v, ok := versions["registry.terraform.io/hashicorp/google"]
	if !ok {
		return nil, fmt.Errorf("error getting google version")
	}
