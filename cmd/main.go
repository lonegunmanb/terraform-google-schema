...
import (
	...
	"github.com/lonegunmanb/terraform-google-schema/v5/schema"
	"golang.org/x/oauth2"
)

func main() {
	refreshFlag := flag.Bool("refresh", false, "Refresh Google schema")
...
func checkGitHubTag(schemaVersion string) (bool, error) {
	...
	for {
		tags, resp, err := client.Repositories.ListTags(context.Background(), "lonegunmanb", "terraform-google-schema", options)
		if err != nil {
			return false, err
		}
